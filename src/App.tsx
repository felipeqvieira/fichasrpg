...
import { 
  Shield, Heart, Zap, Backpack, ScrollText, 
  Edit3, Moon, BookOpen, Ghost, Sword, PawPrint 
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-900 font-sans text-slate-200 pb-20">
        ...
        <div className="max-w-4xl mx-auto shadow-xl rounded-b-lg overflow-hidden border-x border-b border-slate-700/50">
          ...
          <div className="px-4 py-3 bg-red-900/95 text-white flex justify-between items-start backdrop-blur-sm">
            ...
            <div className="flex gap-3 items-center">
              ...
              {/* Botão Lua: Abre o Menu Geral (Descanso, Backup, Config) */}
              <button 
                onClick={() => setIsSettingsOpen(true)}
                title="Menu (Descanso, Backup e Opções)" 
                className="p-1.5 hover:bg-red-800 rounded-lg transition-colors text-red-100 border border-transparent hover:border-red-400/30"
              >
                <Moon size={22} />
              </button>
              ...
              {/* Botão Modo Edição/Leitura */}
              <button 
                onClick={() => setIsEditMode(!isEditMode)}
                className={`p-1.5 rounded-lg transition-colors border ${isEditMode ? 'bg-yellow-500 text-black border-yellow-600 shadow-lg' : 'hover:bg-red-800 text-red-100 border-transparent hover:border-red-400/30'}`}
                title={isEditMode ? "Modo Edição Ativo" : "Ativar Edição"}
              >
                {isEditMode ? <Edit3 size={22} /> : <BookOpen size={22} />}
              </button>
          ...
          <nav className="bg-slate-800 flex justify-center overflow-x-auto no-scrollbar border-t border-slate-700">
            ...
            <TabButton active={activeTab === 'combat'} onClick={() => setActiveTab('combat')} icon={<Sword size={18}/>} label="Combate" />
            <TabButton active={activeTab === 'spells'} onClick={() => setActiveTab('spells')} icon={<Zap size={18}/>} label="Magias" />
            <TabButton active={activeTab === 'features'} onClick={() => setActiveTab('features')} icon={<Heart size={18}/>} label="Habil." />
            <TabButton active={activeTab === 'inventory'} onClick={() => setActiveTab('inventory')} icon={<Backpack size={18}/>} label="Inv." />
            
            <TabButton active={activeTab === 'creatures'} onClick={() => setActiveTab('creatures')} icon={<PawPrint size={18}/>} label="Criat." />
      ...
      <main className="p-4 max-w-4xl mx-auto">
        {activeTab === 'status' && <StatusTab character={character} isEditMode={isEditMode} onUpdate={updateCharacter} />}
        {activeTab === 'combat' && <CombatTab character={character} onUpdateHP={updateHP} onUpdateCharacter={updateCharacter} isEditMode={isEditMode} />}
        {activeTab === 'spells' && <SpellsTab character={character} onUpdate={updateCharacter} isEditMode={isEditMode} />}
        {activeTab === 'features' && <FeaturesTab character={character} onUpdate={updateCharacter} isEditMode={isEditMode} />}
        {activeTab === 'inventory' && <InventoryTab character={character} isEditMode={isEditMode} onUpdate={updateCharacter} />}
        {activeTab === 'creatures' && <CreaturesTab character={character} onUpdate={updateCharacter} isEditMode={isEditMode} />}
        {activeTab === 'notes' && <NotesTab character={character} onUpdate={updateCharacter} isEditMode={isEditMode} />}
      </main>
