...
import { 
  Sword, Shield, Heart, Zap, Crosshair, Skull, 
  AlertTriangle, Check, ChevronDown, Dna, 
  Trash2, Clock, Activity, Footprints, X 
} from 'lucide-react';
import type { CharacterSheet, Feature, Item } from '../../types/dnd';
import { CONDITIONS, DAMAGE_TYPES, SPECIAL_MATERIALS } from '../../data/lists';

// Definindo explicitamente quais chaves da ficha são listas de strings
...
export function CombatTab({ character, onUpdateHP, onUpdateCharacter, isEditMode }: CombatTabProps) {
  ...
  // --- HANDLERS DE AÇÕES (Features & Itens) ---
  ...
  const handleDeleteFeature = (id: string) => {
    if(confirm('Remover esta habilidade?')) {
      onUpdateCharacter?.({ features: character.features.filter(f => f.id !== id) });
    }
  };
  ...
  const renderActionSection = (title: string, icon: any, group: { weapons: Item[], consumables: Item[], features: Feature[] }, forceShow = false) => {
    ...
    return (
      <div className="mb-6 animate-in slide-in-from-bottom-2 duration-300">
        <h3 className="text-xs font-bold text-slate-500 uppercase mb-2 flex items-center gap-2 border-b border-slate-800 pb-1">
          {icon} {title}
        </h3>
        <div className="space-y-2">
          ...
          {group.weapons.map(w => (
            <div key={w.id} className="bg-slate-800 border-l-4 border-slate-600 rounded-r p-2 flex justify-between items-center hover:bg-slate-700/50 transition-colors">
              <div>
                <div className="font-bold text-slate-200 text-sm">{w.name}</div>
                <div className="text-[10px] text-slate-500">{w.damage || '1d4'} • {w.range || '1.5m'}</div>
              </div>
              <div className="text-right">
                <div className="text-sm font-bold text-white bg-slate-700 px-2 py-0.5 rounded border border-slate-600">+5</div>
              </div>
            </div>
          ))}
          ...
          {group.features.map(f => (
            <div key={f.id} className="bg-slate-800 border border-slate-700 rounded p-2 hover:border-indigo-500/50 transition-colors">
              ...
              {/* TRACKER DE USOS (Blocos Grandes) */}
              <div className="flex flex-wrap gap-1.5 justify-end mt-2 pt-2 border-t border-slate-700/30">
                {Array.from({ length: f.maxUses }).map((_, i) => (
                   <button
                     key={i}
                     onClick={() => { 
                       if (i < f.currentUses) handleUseFeature(f); 
                       else if (isEditMode) handleRecoverFeature(f); 
                     }}
                     disabled={!isEditMode && i >= f.currentUses}
                     className={`
                       ...
                       ${(isEditMode || i < f.currentUses) ? 'cursor-pointer' : 'cursor-not-allowed'}
                     `}
                   >
          ...
          {group.consumables.map(c => (
            <div key={c.id} className="bg-slate-800/50 border border-slate-700/50 rounded p-2 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <div className="text-[9px] bg-slate-900 px-1 rounded text-slate-500">x{c.quantity}</div>
              </div>
    ...
    <div className="pb-24">
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
         ...
         <div className="bg-slate-800 p-3 rounded-lg border border-slate-700 text-center shadow-sm flex flex-col items-center justify-center">
           ...
           <div className="text-2xl font-bold text-white leading-none">+{character.initiative}</div>
           <div className="text-[10px] text-slate-500 font-bold uppercase mt-1">Inic</div>
         </div>
         <div className="bg-slate-800 p-3 rounded-lg border border-slate-700 text-center shadow-sm flex flex-col items-center justify-center">
      ...
      <div className="bg-slate-900/50 rounded-xl p-4 border border-slate-800 shadow-sm mb-6">
        ...
        <div className="flex gap-2 mb-4">
           {/* INPUT DE VIDA ATUAL SEM SETAS */}
           <input 
             type="number" 
             placeholder="Valor" 
             className="w-20 bg-slate-950 border border-slate-700 rounded px-2 py-2 text-sm text-center text-white outline-none focus:border-indigo-500 font-bold [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" 
             value={hpInput} 
           ...
           <div className="flex items-center gap-1 bg-slate-950 px-2 rounded border border-slate-800 ml-1">
             ...
             {/* INPUT DE VIDA TEMP SEM SETAS E LARGURA AJUSTADA */}
             <input 
               type="number" 
               className="w-8 bg-transparent text-center text-xs text-white outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" 
               value={character.hp.temp} 
               onChange={e => handleTempHP(parseInt(e.target.value)||0)} 
             />
           </div>
        </div>
      ...
      <div className="bg-slate-800/50 border-l-4 border-slate-600 rounded-r p-2 flex justify-between items-center mb-6 opacity-80 hover:opacity-100">
         ...
         <div className="text-right">
            <div className="text-xs font-bold text-slate-400">+{(character.attributes.strength.value - 10)/2 + character.proficiencyBonus}</div>
         </div>
      </div>
