...
import { 
  ...
  Weight, Search, Coins, Sparkles, Clock
} from 'lucide-react';
import type { CharacterSheet, Item, ItemEffect, ActionType } from '../../types/dnd';
...
const EFFECT_TYPES = [
  ...
  { value: 'speed', label: 'Movimento (Deslocamento)' },
  { value: 'other', label: 'Outro' },
];

const MOVEMENT_LIST = ['Caminhada', 'Voo', 'Escalada', 'Natação', 'Escavação'];
const ATTRIBUTES_LIST = ['strength', 'dexterity', 'constitution', 'intelligence', 'wisdom', 'charisma'];
const SKILLS_LIST = [
...
export function InventoryTab({ character, isEditMode, onUpdate }: InventoryTabProps) {
  ...
  // Atualizado: actionType padrão
  const [formData, setFormData] = useState<Item>({
    id: '', name: '', quantity: 1, weight: 0, type: 'gear', equipped: false, description: '', damage: '', effects: [], actionType: 'none'
  });
  ...
  const handleCurrencyChange = (type: keyof typeof character.currency, value: string) => {
    ...
    onUpdate({ currency: { ...character.currency, [type]: val } });
  };
    ...
    <div className="space-y-6 pb-24">
      <div className="bg-slate-800 p-4 rounded-lg border border-slate-700 shadow-sm">
        ...
        <div className="grid grid-cols-5 gap-2 mb-4">
           {Object.entries(character.currency).map(([key, val]) => (
             <div key={key} className={`flex flex-col items-center bg-slate-900 p-2 rounded border border-slate-700 transition-colors ${isEditMode ? 'focus-within:border-yellow-600' : 'opacity-80'}`}>
               ...
               <input type="number" disabled={!isEditMode} className={`w-full bg-transparent text-center font-bold text-slate-200 outline-none ${!isEditMode && 'cursor-default'}`} value={val} onChange={(e) => handleCurrencyChange(key as any, e.target.value)} />
             </div>
           ))}
        </div>
      ...
      <div className="space-y-3">
        {filteredItems.map(item => {
           ...
           return (
             <div key={item.id} className="bg-slate-800 border border-slate-700 rounded-lg p-3 flex items-start gap-3 shadow-sm group">
               <div className={`p-2 rounded-lg shrink-0 ${item.equipped ? 'bg-green-900/20 text-green-400 border border-green-900/50' : 'bg-slate-900 text-slate-500 border border-slate-700'}`}><TypeIcon size={20} /></div>
               <div className="flex-1 min-w-0">
                 <div className="flex justify-between items-start"><h4 className={`font-bold truncate ${item.equipped ? 'text-green-300' : 'text-slate-200'}`}>{item.name} {item.quantity > 1 && <span className="text-slate-500 text-xs ml-1">x{item.quantity}</span>}</h4></div>
                 <div className="flex flex-wrap gap-2 text-[10px] text-slate-500 mt-1 uppercase font-bold tracking-wide">
                   <span>{ITEM_TYPES[item.type]?.label}</span><span>•</span><span>{item.weight} kg</span>{item.damage && <span className="text-red-400">• {item.damage}</span>}
                   {/* Mostra tag de Ação se tiver */}
                   ...
                   <div className="flex flex-wrap gap-1 mt-2">
                     {item.effects.map((eff, i) => (
                       <span key={i} className="px-1.5 py-0.5 bg-blue-900/30 border border-blue-800 rounded text-[10px] text-blue-200 font-bold flex items-center gap-1">
                         {eff.type === 'ac' ? 'CA' : eff.target?.slice(0,3).toUpperCase() || eff.type} <span className={eff.value >= 0 ? "text-blue-200" : "text-red-300"}>{eff.value >= 0 ? `+${eff.value}` : eff.value}</span>
                       </span>
                     ))}
                   </div>
                 )}
        ...
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-4 sm:p-0">
          ...
          <div className="relative bg-slate-900 border border-slate-700 rounded-xl w-full max-w-md shadow-2xl p-5 animate-in slide-in-from-bottom-10 fade-in max-h-[90vh] overflow-y-auto no-scrollbar">
             ...
             <div className="space-y-4">
               ...
               <div className="flex gap-3"><div className="flex-1"><label className="text-[10px] uppercase font-bold text-slate-500 mb-1 block">Tipo</label><select className="w-full bg-slate-950 border border-slate-700 rounded p-2 text-sm text-white focus:border-red-500 outline-none" value={formData.type} onChange={e => setFormData({...formData, type: e.target.value as any})}>{Object.entries(ITEM_TYPES).map(([key, val]) => (<option key={key} value={key} className={optionClass}>{val.label}</option>))}</select></div><div className="w-24"><label className="text-[10px] uppercase font-bold text-slate-500 mb-1 block">Peso (kg)</label><input type="number" step="0.1" className="w-full bg-slate-950 border border-slate-700 rounded p-2 text-sm text-white focus:border-red-500 outline-none text-center" value={formData.weight} onChange={e => setFormData({...formData, weight: parseFloat(e.target.value) || 0})} /></div></div>
               
               {/* SELETOR DE ACTION TYPE */}
